...
var indexOfArrAllDevices = ArreyOfDevices.length;
var parentMainDiv = document.body.children[2];
function createDevice(name, switches, customises, modes){
   
   indexOfArrAllDevices = ArreyOfDevices.length;
   ArreyOfDevices[indexOfArrAllDevices] = new Device(name); 
   
   for(var itemSw in switches){
      ArreyOfDevices[indexOfArrAllDevices].addPropInObjSwitch(switches[itemSw].name, new Switch(false));
   }
   for(var itemCus in customises){
